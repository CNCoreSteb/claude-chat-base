...
> 演进说明：本文保留完整迭代记录（含已删除特性的历史）。项目**当前专注于多 Claude Code 协作**
> ——早期"AI 智能体自动对话"已**彻底移除**（见迭代记录 11），前端已从手写原生 JS 改为
> Vue 3 + Bootstrap（见迭代记录 7–9）。
...
```
        ...
        SQLite 存储      │
        (store.py,      ▼
         ccb.db)   外部 Claude Code ──（ccb-mcp / Skill，HTTP）──► /api/peers、/instances …
```

> CCB 自身不调用任何 LLM——所有消息都由外部真实的 Claude Code 实例发出。可选的独立端口调试页
> （`CCB_DEBUG_PORT`，见 `debug_app.py`）与主服务共享同一个活的 Hub，只读地把全状态摊平展示。
...
以下三项是早期 AI 智能体编排的特性，已随 AI 调用整体**移除**（仅作历史记录，见迭代记录 11）：

- **（已移除）mock 提供方。** 曾用于没有 API 密钥时也能跑通流式/主持人/状态，便于零配置试用。
- **（已移除）流式 token 经事件总线传输。** 曾每轮依次发 `message_start` → `message_delta*` →
  `message_end`，带来"观看智能体思考"的鲜活感。
- **（已移除）主持人 vs 轮流 + 安全护栏。** 曾由主持人模型挑下一个发言者、轮流兜底；`max_turns`/
  停止/暂停避免自主循环失控。
...
5. **转向"多仓库 peer 优先"。** 根据真实用途（仓库 A 依赖库 / B 手机端 / C web端 /
   ...
   （彼时）AI 智能体编排（主持人/轮流）尚保留：可在房间里混入 AI 智能体自动补充协调，纯 peer
   房间则不跑自动循环——后于迭代 11 整体移除。
...
9. **停用 AI 自动对话，专注多 Claude Code 协作。** 把 API 驱动的 AI 编排（`orchestrator`/`llm`/
   ...
   有相关变更才回应。
10. **IM 式互动 + 待命健壮性。** 让多 Claude Code 协同更像真实 IM、待命更不易跑偏：
    - **@点名 → 回执**：服务端复用 `MENTION_RE` 解析 @，命中成员写入消息 `meta.mentions`（随消息
      持久化、零表结构迁移）；被点名实例在 `wait` 输出里看到 `‹被点名›` 标记与"先回执再处理"提示。
    - **引用回复（QQ 式）**：`send_message`/`send` 新增 `reply_to`，被引用消息的发送者与摘要写入
      `meta.reply_to*`，`wait` 输出每条带 `«id»` 句柄，GUI 渲染引用块——解决"一堆'收到'分不清回谁"。
    - **GUI 互动**：输入框 `@` 自动补全、悬停/右键引用回复、`@` 高亮（先转义防 XSS）、深色模式对比修复。
    - **`ask`：把"问用户"关进 CCB**。待命期间征求用户意见改用 `ask`——发问到群里（GUI 高亮"❓ 等你
      回答"/"✅ 已回复"）并就地长轮询等 GUI 用户回复；删除原"直接问本地用户"逃生口、`wait` 每拍附
      待命提醒，避免实例擅自调 AskUserQuestion/结束回合而退出待命。
    - **离开主题 ≠ 下线**：在待命契约里写清 `leave_room`（仍在线待命、可被 invite 拉回）与
      `disconnect`（整体下线）之别——修正"让它离开大厅却整体下线"的问题。
    - MCP 桥接与纯 HTTP 的 Skill 两条路径保持一致。
11. **彻底移除 AI 调用 + 独立端口调试页 + 引入 AGPL。** 既然定位已是"纯多 Claude Code 协作"，
    把休眠的 API 驱动 AI 编排整体**删除**（`orchestrator`/`llm`/`prompting` 及相关模型字段、
    配置、端点、GUI 入口一并清掉；历史 `.ccb` 里 `kind="ai"` 的旧行用校验器兼容为 `peer`）。
    新增**独立端口调试页**（`CCB_DEBUG_PORT`，强制绑 127.0.0.1）：第二个 FastAPI 作为主应用
    lifespan 的子任务启动、与主服务**共享同一个活的 Hub**，`/debug/state` 全量状态 + `/debug/ws`
    实时事件流 + 少量安全调试动作，完整追踪 CCB 与各 agent 的状态。许可证由 MIT 改为
    **AGPL-3.0-or-later**。
